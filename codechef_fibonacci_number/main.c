...
#if 1
COMUN_FUNC_STATICA entero_largo
e216_algoritmo_euclidiano_extendido (entero_largo a, entero_largo b,
...
COMUN_FUNC_STATICA natural
e216_inverso_multiplicativo_modular (entero_largo a, entero_largo m)
{
    entero_largo r = 0;
    entero_largo x;
...
COMUN_FUNC_STATICA entero_largo
e216_simbolo_jacobi (entero_largo a, entero_largo n)
{
    assert_timeout (n & 1);
    entero_largo t = 1;
...
#if 1
COMUN_FUNC_STATICA entero_largo
paso_bebe_paso_gigante (entero_largo a, entero_largo b, entero_largo g)
{
    entero_largo r = -1;
    ...
    for (natural x = 1; x <= Q; x++)
    {
        ...
        hm_iter iter =
        hash_map_robin_hood_back_shift_obten (ht, (void *) a_Q_x, &x_tmp);
        if (iter == HASH_MAP_VALOR_INVALIDO)
        {
            hash_map_robin_hood_back_shift_insertar_nuevo (ht, (void *) a_Q_x,
                                                           (entero_largo) x);
    ...
    for (natural y = 0; y <= Q; y++)
    {
        entero_largo x = 0;
        hm_iter iter =
        hash_map_robin_hood_back_shift_obten (ht, (void *) b_a_y, &x);
...
int
codechef_fibonacii_number_compara (const void *pa, const void *pb)
{
    int r = 0;
...
COMUN_FUNC_STATICA entero_largo
codechef_fibonacci_valida (entero_largo n, entero_largo p, entero_largo x,
                           entero_largo y)
{
    entero_largo x_inv = e216_inverso_multiplicativo_modular (x, p);
    entero_largo dos_inv = (p + 1) >> 1;
    entero_largo y_inv =
    primalidad_mul_mod (primalidad_normalizar_signo_modulo (1 - x, p),
                        dos_inv, p);
    //    entero_largo r=((primalidad_exp_mod(y,n,p)-primalidad_exp_mod(y_inv,n,p))*x_inv)%p;
    entero_largo r =
    primalidad_mul_mod (primalidad_normalizar_signo_modulo
                        (primalidad_exp_mod (y, n, p) -
                         primalidad_exp_mod (y_inv, n, p), p), x_inv, p);
    return r;
}

COMUN_FUNC_STATICA bool
codechef_fibonacci_valida_no_hay_solucion (entero_largo p, entero_largo x,
                                           entero_largo y, entero_largo c)
{
    bool r = verdadero;
...
COMUN_FUNC_STATICA entero_largo_sin_signo
codechef_fibonacci_number_core (entero_largo_sin_signo c,
                                entero_largo_sin_signo p)
{
    ...
    shanks_tonelli_conguencia_residuo_cuadratico (5, p, pd);
    ...
    while (d * d <= p_menos_1)
    {
        ...
        if ((!(p_menos_1 % d_inv) && primalidad_exp_mod (y, d_inv, p) == 1)
            && (m == COMUN_VALOR_INVALIDO || d_inv < m))
        {
            m = d_inv;
        }
    ...
    for (natural impar = falso; impar < 2; impar++)
    {
        ...
        entero_largo_sin_signo discriminante =
        primalidad_normalizar_signo_modulo (primalidad_mul_mod (z, z, p) -
                                            4 * factor_discriminante, p);
        entero_largo simbolo_jacobi =
        shanks_tonelli_simbolo_jacobi (discriminante, p);
        ...
        if (simbolo_jacobi >= 0)
        {
            ...
            entero_largo ys[] = {
                primalidad_mul_mod (primalidad_normalizar_signo_modulo
                                    ((entero_largo) z - (entero_largo) u, p),
                                    (p + 1) >> 1, p),
                primalidad_mul_mod ((entero_largo) z + (entero_largo) u, (p + 1) >> 1,
                                    p)
            };
            entero_largo ns[2] = { 0 };
            natural ns_cnt = 0;
            ...
            {
                entero_largo n = paso_bebe_paso_gigante (y, ys[i], p);
                if (n >= 0)
                {
                    ns[ns_cnt++] = n;
                }
            }
            qsort (ns, ns_cnt, sizeof (entero_largo), comun_compara_entero_largo);
            for (natural i = 0; i < ns_cnt; i++)
            {
                entero_largo n = ns[i];
                entero_largo n_tmp=COMUN_VALOR_INVALIDO;
                if (impar)
                {
                    if (n & 1)
                    {
    ...
    r =
    comun_min ((entero_largo_sin_signo) n_par,
               (entero_largo_sin_signo) n_impar);
